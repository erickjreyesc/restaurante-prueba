<x-jet-action-section>
    <x-slot name="title">
        {{ __('frontend.session-info') }}
    </x-slot>

    <x-slot name="description">
        {{ __('frontend.session-desc') }}
    </x-slot>
            ...
            <div class="mt-3">
                ...
                @foreach ($this->sessions as $session)
                    <div class="d-flex">
                        ...
                        <div class="ms-2">
                            <div>
                                {{ $session->agent->platform() ? $session->agent->platform() : __('frontend.unknown') }} - {{ $session->agent->browser() ? $session->agent->browser() : __('frontend.unknown') }}
                            </div>
                            ...
                            <div>
                                <div class="small font-weight-lighter text-muted">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-success font-weight-bold">{{ __('frontend.session-this') }}</span>
                                    @else
                                        {{ __('frontend.session-last') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
